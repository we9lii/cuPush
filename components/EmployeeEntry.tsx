...
import { Save, AlertCircle, CheckCircle, Smartphone, MapPin, Zap, FileText, User as UserIcon, List, Edit, X, RefreshCw, ChevronDown, Droplet, Link as LinkIcon, Copy, ExternalLink, Settings } from 'lucide-react';
...
export const EmployeeEntry: React.FC<Props> = ({ currentUser }) => {
    const [formData, setFormData] = useState({
        clientName: '',
        mobileNumber: '',
        region: '', // Changed to empty string for text input
        systemSizeHp: '',
        wellsCount: '',
        projectMapUrl: '',
        pricePerHp: '',
    ...
    const handleEditClick = (client: ClientRecord, e: React.MouseEvent) => {
        e.stopPropagation(); // Prevent row click
        setFormData({
            clientName: client.clientName,
            mobileNumber: client.mobileNumber,
            region: client.region,
            systemSizeHp: client.systemSizeHp?.toString() || '',
            wellsCount: client.wellsCount?.toString() || '',
            projectMapUrl: client.projectMapUrl || '',
            pricePerHp: client.pricePerHp?.toString() || '',
        ...
        try {
            if (isEditing && editingId) {
                ...
                await StorageService.updateClient(editingId, {
                    ...
                    systemSizeHp: Number(formData.systemSizeHp),
                    wellsCount: Number(formData.wellsCount),
                    projectMapUrl: formData.projectMapUrl,
                    pricePerHp: Number(formData.pricePerHp),
    ...
    const handleSettingsSubmit = async (data: { email?: string, mobileNumber?: string, password?: string }) => {
        try {
            await StorageService.updateUserProfile(currentUser.id, data);
        ...
        <div className="max-w-4xl mx-auto py-4 px-4 space-y-8">
            ...
            <div className="flex justify-end">
                <button
                    ...
                    className="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-agri-600 dark:hover:text-agri-400 transition-colors"
                >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">اسم العميل <span className="text-red-500">*</span></label>
                                ...
                                {/* Region */}
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">المنطقة <span className="text-red-500">*</span></label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-agri-500">
                                ...
                                {/* System Size (HP) */}
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">حجم النظام (حصان) <span className="text-red-500">*</span></label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-agri-500">
                                        ...
                                        <input
                                            ...
                                            className="block w-full pr-10 bg-gray-50 dark:bg-slate-700 border-gray-300 dark:border-slate-600 text-gray-900 dark:text-white rounded-lg focus:ring-agri-500 focus:border-agri-500 py-3 border shadow-sm transition-all placeholder-gray-400 dark:placeholder-gray-500"
                                            placeholder="مثال: 50"
                                            value={formData.systemSizeHp}
                                            onChange={e => setFormData({ ...formData, systemSizeHp: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                {/* Wells Count */}
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">عدد الآبار <span className="text-red-500">*</span></label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-agri-500">
                                        ...
                                        <input
                                            ...
                                            onChange={e => setFormData({ ...formData, wellsCount: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                {/* Location (Map URL) */}
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">الموقع (رابط Google Maps)</label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-agri-500">
                                        ...
                                        <input
                                            ...
                                            value={formData.projectMapUrl}
                                            onChange={e => setFormData({ ...formData, projectMapUrl: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                {/* Price */}
                                <div className="col-span-2 md:col-span-1">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">السعر (ر.س) <span className="text-red-500">*</span></label>
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-agri-500">
                                            <img
                                        ...
                                        <input
                                            ...
                                            value={formData.pricePerHp}
                                            onChange={e => setFormData({ ...formData, pricePerHp: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="col-span-2">
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">آخر تحديث / ملاحظات <span className="text-red-500">*</span></label>
                                    <textarea
                                        rows={3}
                                        required
                    ...
                    <table className="w-full text-right">
                        <thead className="bg-gray-50 dark:bg-slate-700/50 text-gray-600 dark:text-gray-300 text-sm">
                            <tr>
                                ...
                                <th className="p-4">الحجم (حصان)</th>
                                <th className="p-4">عدد الآبار</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-slate-700 text-sm">
                            {myClients.length === 0 ? (
                                ...
                                myClients.map(client => (
                                    <tr key={client.id} className={`hover:bg-blue-50/50 dark:hover:bg-slate-700/30 transition-colors cursor-pointer ${editingId === client.id ? 'bg-blue-50 dark:bg-slate-700/50' : ''}`} onClick={() => setViewClient(client)}>
                                        ...
                                        <td className="p-4 text-gray-600 dark:text-gray-400">{client.systemSizeHp ?? '-'}</td>
                                        <td className="p-4 text-gray-600 dark:text-gray-400">{client.wellsCount ?? '-'}</td>
                                        <td className="p-4 text-gray-600 dark:text-gray-400">
                                            {client.projectMapUrl ? (
                                                <div className="flex gap-1">
                                                    <button
                                                        className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 text-[10px] bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded"
                                                        title="نسخ الرابط"
                                                    ...
                                                    <a
                                                        ...
                                                        className="text-green-600 dark:text-green-400 hover:underline flex items-center gap-1 text-[10px] bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded"
                                                        title="فتح الموقع"
                                        ...
                                        <td className="p-4 text-gray-600 dark:text-gray-400">{client.pricePerHp ? client.pricePerHp.toLocaleString() : '-'}</td>
